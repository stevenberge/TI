...
void BluetoothProcess::set_led_state(cwiid_wiimote_t *wiimote, unsigned char led_state)
{
...
void BluetoothProcess::set_rpt_mode(cwiid_wiimote_t *wiimote, unsigned char rpt_mode)
{
...
void BluetoothProcess::print_state(struct cwiid_state *state)
{
        int i;
        int valid_source = 0;
        ...
        */
        ...
        for (i = 0; i < CWIID_IR_SRC_COUNT; i++) {
                if (state->ir_src[i].valid) {
                        ...
                        emit sendData( state->ir_src[i].pos[CWIID_Y], state->ir_src[i].pos[CWIID_X] );
                        break;
                }
        }
        /*if (!valid_source) {
...
void BluetoothProcess::stopProcess()
{
    stopped = true;
}
